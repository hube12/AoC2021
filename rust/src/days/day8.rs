...
fn find_easy_segments(segments: &Vec<&str>) -> anyhow::Result<HashMap<usize, String>> {
    ...
    let one = segments.iter().find(|x| x.len() == 2).ok_or(anyhow::Error::msg("Missing 7"))?;
    insert_map(&mut map, 1, *one);
    let seven = segments.iter().find(|x| x.len() == 3).ok_or(anyhow::Error::msg("Missing 7"))?;
    insert_map(&mut map, 7, *seven);
    let four = segments.iter().find(|x| x.len() == 4).ok_or(anyhow::Error::msg("Missing 7"))?;
    insert_map(&mut map, 4, *four);
    let eight = segments.iter().find(|x| x.len() == 7).ok_or(anyhow::Error::msg("Missing 7"))?;
...
fn find_0_6_9(segments: &Vec<&str>, map: &mut HashMap<usize, String>) -> anyhow::Result<()> {
    ...
    let nine = zero_nine_six.iter().find(|&unknown| four.chars().all(|x| unknown.contains(x))).ok_or(anyhow::Error::msg("Missing 9"))?;
    insert_map(map, 9, nine);
    let one = map.get(&1).ok_or(anyhow::Error::msg("Missing 1"))?;
    let six = zero_nine_six.iter().find(|&unknown| !one.chars().all(|x| unknown.contains(x))).ok_or(anyhow::Error::msg("Missing 6"))?;
    insert_map(map, 6, six);
    let zero = zero_nine_six.iter().filter(|&x| x != six && x != nine).next().ok_or(anyhow::Error::msg("Missing 0"))?;
...
fn find_2_3_5(segments: &Vec<&str>, map: &mut HashMap<usize, String>) -> anyhow::Result<()> {
    ...
    let one = map.get(&1).ok_or(anyhow::Error::msg("Missing 1"))?;
    let three = two_three_five.iter().find(|&unknown| one.chars().all(|x| unknown.contains(x))).ok_or(anyhow::Error::msg("Missing 3"))?;
    insert_map(map, 3, three);
    let six = map.get(&6).ok_or(anyhow::Error::msg("Missing 6"))?;
    let five = two_three_five.iter().filter(|&x| x != three)
        .find(|&x| x.chars().filter(|&x| six.contains(x)).count() == 5).ok_or(anyhow::Error::msg("Missing 5"))?;
    insert_map(map, 5, five);
    let two = two_three_five.iter().filter(|&x| x != three && x != five).next().ok_or(anyhow::Error::msg("Missing 2"))?;
...
fn match_digits_with_representation<'a, I: IntoIterator<Item=&'a String>>(segments: I, digit: &&str) -> bool {
    segments.into_iter().any(|x: &String| match_sequence(&x.as_str(), digit))
}
...
impl Solution1 for Day8 {
    fn run_solution1(&self, lines: Vec<String>) -> anyhow::Result<String> {
        let puzzles: Vec<(Vec<&str>, Vec<&str>)> = lines.iter()
            .map(|x| x.split_once(" | ").ok_or(anyhow::Error::msg("Can not divide into two parts")))
            .map(|x| x.and_then(|(segments, digits)| Ok((segments.split_whitespace().collect(), digits.split_whitespace().collect()))))
            .collect::<Result<_, _>>()?;
...
impl Solution2 for Day8 {
    fn run_solution2(&self, lines: Vec<String>) -> anyhow::Result<String> {
        ...
        for (segments, digits) in &puzzles {
            ...
            for digit in digits{
                let digit=*map.get(&*sort_str(*digit)).ok_or(anyhow::Error::msg("Missing digit"))?;
